...
{
	int start_opint = 0;
	for (int id = 0; id < TLB[Table_length * threadID]; id++)			// TLB[2*id+1]= The number of elements this thread will execute
		start_opint = start_opint + TLB[Table_length * id + 1];		//start_opint= before this thread how many element has been execute ------> thread_Table_length's start point is  thread_1's number of element  + thread_2's number of element	
...
DWORD WINAPI Thread_dynamic(LPVOID lpParam) 
{
	...
	//printf("Thread %3d start point is %d  execute to %d\n",id, TLB[Table_length * id + 2], TLB[Table_length * id + 2] +TLB[Table_length * id+1]-1); //Test Each thread can get their own variable
	
	for (int element = TLB[Table_length * id + 2]; element < TLB[Table_length * id + 2] + TLB[Table_length * id+1]; element++)
	{	
...
{	
	for (int i = 0; i < thread_num; i++)
		printf("Thread %2d start from: %2d to %2d of elements  Execute %4d elements\n", i, TLB[Table_length * i + 2], TLB[Table_length * i + 1]+ TLB[Table_length * i + 2]-1, TLB[Table_length * i + 1]);
}
...
void thread_input(void)
{
	...
	scanf("%d", &thread_num);
	TLB = new int[Table_length * thread_num];		// calculate  number of element  for per thread
...
void assign()
{
	...
	for (int i = 0; i < thread_num; i++)
	{
		TLB[Table_length*i] = i;
		TLB[Table_length*i+1] = (size*size) / thread_num;
	}	
	for (int i = remain; i > 0; i--)
		TLB[Table_length*(remain - i)+ 1] = TLB[Table_length*(remain - i)+ 1] + 1;

	for (int i = 0; i < thread_num; i++)
		TLB[Table_length * i + 2] = start_opint_calculate(i);
}
void print_input()
{
...
//--Threading program & confirm every thread can create  within the limitation---//
	...
	for (int i = 0; i < thread_num; i++)
	{
		thread[i] = CreateThread(NULL, 0, Thread_dynamic, (void*)TLB[Table_length * i], 0x00000004, &thread_id[i]); //(void*) to make variable compactable to the function	
